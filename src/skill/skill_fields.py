...
from util.util import append_sql_files, get_attribute_string, get_skill_effect_type, get_skill_target_type, load_array_from_file, str_format, write_array_to_file
...
def __add_skill_effect(skill_id, skill_effects, effect_ids, description):
    ...
    for index, effect in enumerate(effect_ids):
        ...
        skill_effects.append(Skill_Effect(
            skill_id=skill_id, effect_id=effect['id'], value=effect['value'], index=skill_index))
...
def __create_sql_files(translations, skills, skill_effects):
    ...
    append_sql_files(scripts=['skill_translation_key', 'skill_translation', 'skill',
                     'skill_effects'], appended_filename='appended_skill', language=language)
...
def create_skill_sql(locale: str):
    ...
    with open(Path(db_filepath + 'skill.json').absolute(), encoding="utf8") as f:
        ...
        for obj in d:
            id = obj['id']
            if str(id) not in included_skills:
                continue
            description = obj['description']
            linked_id = re.search(r"\{hyperlink_id (\d+)\}", description)
            linked_skill = None
            if linked_id:
                linked_skill = __get_linked_skill(
                    hyperlinks, linked_id.group(1))
            filtered_desc = re.sub(
                r'\{hyperlink_id (\d+)\}', '{linked_skill}', description)
            translations.append(Translation(
                id=f'{tl_id_preval}_{id}_N', language=language, text=str_format(obj['name'])))
            translations.append(Translation(
                id=f'{tl_id_preval}_{id}_D', language=language, text=str_format(filtered_desc)))
            ...
            skills.append(Skill(description=f'{tl_id_preval}_{id}_D',
                                ...
                                attribute=get_attribute_string(
                                    attribute[0]) if attribute else None,
                                skill_type=get_skill_effect_type(
                                    obj['skill_effect_type']),
                                target_type=get_skill_target_type(
                                    obj['skill_target_type']),
                                name=f'{tl_id_preval}_{id}_N'))
            __add_skill_effect(skill_id=id, skill_effects=skill_effects,
                               effect_ids=obj['effects'], description=filtered_desc)
            included_effects = included_effects + [int(item['id']) for item in obj['effects']]
    f.close()
    write_array_to_file(included_effects, 'skill_effects')
    __create_sql_files(translations, skills, skill_effects)
