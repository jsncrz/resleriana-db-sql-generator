...
from util.util import append_sql_files, get_skill_type, str_format, write_array_to_file
...
def __add_chara_tls(tl_id_preval, translations, obj, id):
    translations.append(Translation(
        id=f'{tl_id_preval}_{id}_AT', language=language, text=str_format(obj['acquisition_text'])))
    translations.append(Translation(
        id=f'{tl_id_preval}_{id}_AN', language=language, text=str_format(obj['another_name'])))
    translations.append(Translation(
        id=f'{tl_id_preval}_{id}_D', language=language, text=str_format(obj['description'])))
    translations.append(Translation(
        id=f'{tl_id_preval}_{id}_FN', language=language, text=str_format(obj['fullname'])))
    translations.append(Translation(
        id=f'{tl_id_preval}_{id}_N', language=language, text=str_format(obj['name'])))
...
def __add_tags_and_tls(translations, tags):
    with open(Path(db_filepath + 'character_tag.json').absolute(), encoding="utf8") as f:
        ...
        for obj in d:
            id = obj['id']
            translations.append(Translation(
                id=f'TAG_{id}_N', language=language, text=str_format(obj['name'])))
            tags.append(Tag(
...
def __add_character_tags(character_id, character_tags, tag_ids):
    for tag in tag_ids:
        character_tags.append(Character_Tag(
            character_id=character_id, tag_id=tag))
    ...
    for index, skills in enumerate(skill_ids):
        for skill in skills:
            character_skills.append(Character_Skill(
                character_id=character_id, skill_id=skill, skill_type=get_skill_type(index)))

def __add_resistances_to_array(resists, obj, id):
...
def __add_stats_to_array(stats, obj, id):
...
def __add_character_to_array(tl_id_preval, charas, obj, id):
...
def __create_sql_files(translations, charas, stats, resists, tags, character_tags, character_skills):
    ...
    append_sql_files(scripts=['chara_translation_key', 'chara_translation', 'tag', 'character', 'character_tag',
                     'character_skill', 'character_stat', 'character_resist'], appended_filename='appended_chara', language=language)
...
def create_character_sqls(locale: str):
    ...
    with open(Path(db_filepath + 'character.json').absolute(), encoding="utf8") as f:
        ...
        for obj in d:
            ...
            __add_character_to_array(tl_id_preval, charas, obj, id)
            # Add tags objects
            __add_character_tags(id, character_tags, obj['tag_ids'])
            # Add Character skills
            __add_character_skills(id, character_skills, [
                                   obj['normal1_skill_ids'], obj['normal2_skill_ids'], obj['burst_skill_ids']])
            skill_list = skill_list + obj['normal1_skill_ids'] + obj['normal2_skill_ids'] + obj['burst_skill_ids']
            # Add stats objects
            __add_stats_to_array(stats, obj, id)
            # Add resistance objects
            __add_resistances_to_array(resists, obj, id)
    f.close()
    write_array_to_file(skill_list, 'character_skills')
    __create_sql_files(translations, charas, stats, resists, tags, character_tags, character_skills)
