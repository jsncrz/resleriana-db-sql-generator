...
def memoria_role_sql(memoria_roles: list[Memoria_Role]):
    insert_string = 'INSERT INTO `MEMORIA_ROLE`(`MEMORIA_ID`,`ROLE`,`CREATE_DATE`) VALUES '
    for memoria_role in memoria_roles:
        insert_string += ("".join([
            f"({memoria_role.memoria_id},\n\t",
            f"'{memoria_role.role}',\n\t",
            f"'{memoria_role.create_date}'),\n\t"]))
    sql_file = open("sql/memoria_role.sql", encoding="utf-8", mode="w")
